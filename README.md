...
- 基于软件工程专业 2023 级普通推免拟录取名单的匿名数值样本，对照当前 GPA 与综测成绩的历史分位、入围区间、目标差距和多种提升情景；不把历史样本冒充真实录取概率。
- 参考软件学院 2023 级综测折合方法输入奖项、级别、作者顺序、学年、任职考评或志愿时长并自动计分，支持限项、学年/类别封顶、同项取高和当前 GPA 叠加。
- 按推免表竞赛分类汇总 2026 实际赛程，并依据 2025/2026 节奏和表列月份给出明确标记的 2027 预计时间；支持按年份、类别、状态和时间排序。
- 本站登录态、课程、规则偏好和综测项目持久化；支持主动退出但保留个人数据。
- 课程成绩与综测项目 CSV 导入/导出，重复导入按课程代码或综测项目参数合并；另支持完整 JSON 导出。
